...
public class P6 {
	...
	public int process(PrintWriter p) {
	...
	public void run(PrintWriter p) {
		int resultCode = process(p);
	...
	public static void main(String[] args) {
		...
		try {
            Codegen.p = new PrintWriter(args[1]); // generate printwriter
        } catch (Exception ex) {
            System.err.println("Exception caught: generator print writer");
        }
		instance.run(Codegen.p);
		Codegen.p.close();
	}
}
